-- Engagement Report: Compare engagement metrics between sessions with/without Mula (Optimized)
...
-- Get engagement metrics per session (simplified approach)
session_metrics AS (
  SELECT
    bs.host,
    bs.sessionId,
    bs.cohort,
    -- Use max time_on_page as fallback for session duration
    MAX(CASE WHEN wl.properties['eventName'] = 'time_on_page' THEN CAST(wl.properties['eventValue'] AS INTEGER) END) AS time_on_page_seconds,
    -- Max scroll depth in pixels
    MAX(CASE WHEN wl.properties['eventName'] = 'scroll_depth_pixels' THEN CAST(wl.properties['eventValue'] AS INTEGER) END) AS scroll_depth_pixels
  FROM base_sessions bs
  INNER JOIN mula.webtag_logs wl 
    ON bs.host = wl.properties['host'] 
    AND bs.sessionId = wl.properties['sessionId']
    AND datehour >= date_format(current_timestamp - interval '{{days_back}}' day, '%Y/%m/%d/%H')
  WHERE wl.properties['eventName'] IN ('time_on_page', 'scroll_depth_pixels')
  GROUP BY bs.host, bs.sessionId, bs.cohort
),
